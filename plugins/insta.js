...
const LOAD_ING = "*ᴡᴀɪᴛ ʙʀɪ ɪᴀᴍ sᴇᴀʀᴄʜɪɴɢ🔍*"
const UPLOAD_ING = "*ᴋᴀɴᴅᴜᴘɪᴅɪᴄʜᴇ 🧞‍♀️* \n\n\n *Downloading & uploading in process🧚‍♀️...*"
...
Asena.addCommand({pattern: 'insta ?(.*)', fromMe: false, desc: "Downloads from instagaram" , dontAddCommandList: true }, async (message, match) => {
   ...
   const { status, type, data } = await instagram(match[1], 'julie')
    ...
    if (type === 'image') return await message.sendMessage(data, MessageType.image, { caption: "*Downloaded by 💘Pikachu💘*", quoted: message.data })
    
    if (type === 'video') return await message.sendMessage(data, MessageType.video, { caption: "*Downloaded by 💘Pikachu💘*", quoted: message.data })
